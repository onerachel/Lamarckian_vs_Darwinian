...
class LocalRunnerTrain(Runner):
        ...
        while (time := data.time) < simulation_time and timestep <= NUM_STEPS:
            ...
            if timestep == 0  or time >= last_control_time + control_step:
                ...
                if timestep > 0:
                    ...
                    new_position = results.environment_states[-1].actor_states[0].position
                    
                    # compute the rewards from the new and old positions of the agents
                    reward = cls._calculate_velocity(old_position, new_position)
    ...
    async def run_batch(
        ...
        buffer = Buffer(obs_dims, num_joints, num_agents)
