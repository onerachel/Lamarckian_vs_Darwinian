...
from revolve2.core.modular_robot import ActiveHinge, Body, Core, Brick, Module, NotFinalizedError
...
class MorphologicalMeasures:
    ...
    def __calculate_xy_symmetry(self) -> float:
        ...
        num_symmetrical = 0
        max_offset = min(self.core_grid_position[2], self.bounding_box_height-1-self.core_grid_position[2])
        for x in range(self.bounding_box_depth):
            for y in range(self.bounding_box_width):
                for z in range(1, max_offset+1):
    ...
    def __calculate_xz_symmetry(self) -> float:
        ...
        num_symmetrical = 0
        max_offset = min(self.core_grid_position[1], self.bounding_box_width-1-self.core_grid_position[1])
        for x in range(self.bounding_box_depth):
            for z in range(self.bounding_box_height):
                for y in range(1, max_offset+1):
    ...
    def __calculate_yz_symmetry(self) -> float:
        ...
        num_symmetrical = 0
        max_offset = min(self.core_grid_position[0], self.bounding_box_depth-1-self.core_grid_position[0])
        for y in range(self.bounding_box_width):
            for z in range(self.bounding_box_height):
