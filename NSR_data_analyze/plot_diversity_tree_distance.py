...
# Read files
df = pd.read_csv(path+"/NSR_data/"+f'{version}'+"_individual_tree_distance.csv")
...
_, p_value = ttest_ind(Lamarckian_Learning['dist'], Darwinian_Learning['dist'], equal_var=False)
...
for i, file in enumerate(data):
    describe = data[i].groupby(['generation']).describe()['dist']
    # print(describe)
    mean = describe['mean']
    std = describe['std']
    max = describe['max']
...
figure.savefig(path+"/NSR_data/plot_images/fitness_diversity_"+f'{version}'+".png", bbox_inches='tight')
