...
async def main() -> None:
    ...
    folder = '../experiment_lamarckian_darwinian'

    for exp in ('darwinian', 'lamarckian'):
        print(exp)

        parent1_dist = np.zeros(1500*10)
        idx = 0

        for ran_num in range(1,11):
            print(ran_num)

            db = open_database_sqlite(folder + '/point_navigation/' + exp + '/run' + str(ran_num))
            df = pd.read_sql(
                select(
                    DbEAOptimizer,
                    DbEAOptimizerGeneration,
            ...
            for i, ind in df.iterrows():
                ...
                if parents.shape[0] > 0:
                    ...
                    db = open_async_database_sqlite(folder + '/point_navigation/' + exp + '/run' + str(ran_num))
                    ...
                    bd = BodyDevelop(10, 10, child_genotype.body, child_genotype.random_seed)
                    child_body = bd.develop()[0]
                    child_tree = body_to_tree(child_body)

                    bd = BodyDevelop(10, 10, parent1_genotype.body, parent1_genotype.random_seed)
                    parent1_body = bd.develop()[0]
        ...
        df = pd.read_csv('../experiment_lamarckian_darwinian/point_navigation/' + exp + '/summary.csv')
        df['parent_distance'] = parent1_dist
        df.to_csv('../experiment_lamarckian_darwinian/point_navigation/' + exp + '/new_new_summary.csv')
